...
class DealingWithErrorsExample {
  ...
  public static void tryCatchWithoutFinallyExample() {
    try {
      Reader reader = new FileReader("missing.file", StandardCharsets.UTF_8);
      Writer writer = new FileWriter("output.file", StandardCharsets.UTF_8);
  ...
  public static void tryCatchFinallyExample() {
    ...
    try {
      reader = new FileReader("missing.file", StandardCharsets.UTF_8);
      writer = new FileWriter("output.file", StandardCharsets.UTF_8);
  ...
  public static void tryWithResourcesExample() {
    try (Reader reader = new FileReader("missing.file", StandardCharsets.UTF_8);
        Writer writer = new FileWriter("output.file", StandardCharsets.UTF_8)) {
      writer.write(reader.read());
    } catch (IOException e) {
      System.out.println("Exception: " + e);
